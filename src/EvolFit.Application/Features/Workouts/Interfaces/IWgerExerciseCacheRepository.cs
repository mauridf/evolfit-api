...
public interface IWgerExerciseCacheRepository
{
    ...
    Task<IReadOnlyList<WgerExerciseCache>> SearchByTermsAsync(
        IReadOnlyCollection<string> terms, string languageMode, int limit, CancellationToken ct = default);
    Task<DateTime?> MaxCatalogExpiresAtAsync(CancellationToken ct = default);
    Task RemoveCatalogEntriesAsync(CancellationToken ct = default);
